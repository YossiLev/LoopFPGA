...
(
    ...
    input   wire [31:0] i_config   ,  // S/W      = originated configuration (which includes optional reset signal).
                                      ...
                                      //             resolution DAC.
                                      //
                                      // bit[9]    = "1" ==> square input before use
                                      //             "0" ==> use input as-is.
                                      //
    ...
    //
    // ---------------------------------------------------------------
    // Recorder ports (i3 bus)
    // ---------------------------------------------------------------
    // Config/control (reg_clk domain):
    //   i3_cs[0] write : {i3_data_in[12]=enable, [11:6]=rec_index_B, [5:0]=rec_index_A}
    //                    Writing enable=1 also resets both the capture counter
    //                    and the interval divider counter to 0.
    //   i3_cs[1] write : i3_data_in[9:0] = rec_read_addr (word address for readback)
    //   i3_cs[2] write : i3_data_in[31:0] = rec_interval
    //                    Number of logic_clk ticks between successive buffer writes.
    //                    1 = record every tick, 2 = every 2nd tick, etc.
    //                    0 is treated the same as 1 (always record).
    //   i3_cs[3] read  : recorder_buf_A[rec_read_addr] -> o3_data_out
    //   i3_cs[4] read  : recorder_buf_B[rec_read_addr] -> o3_data_out
    //   i3_cs[5] read  : {21'b0, rec_counter[9:0], rec_enable} -> o3_recorder_status
    //
    input    wire [15:0] i3_cs,
    input    wire        i3_write,
    input    wire        i3_read,
    input    wire [31:0] i3_data_in,

    output   reg  [31:0] o3_data_out,
    output   wire [31:0] o3_recorder_status
);
...
//
// Source-select MUX.
// To change the capture list, edit ONLY this function.
// The 4-bit index maps to any internal register you choose.
// Add/remove entries freely; nothing else needs to change.
//
...
// (which this design avoids — writes happen on logic_clk, reads are
...
reg     [15:0]    i0_shift;    // Amount of right shift for the integrated multiplication
                                     ...
                                     // can be very small
reg     [31:0]    output_counter_1_limit;    // For high precision PWM output shaping:
...
reg     [4:0]     output_precision_size;    // # of bits from the _unshifted output_ which are used to modulate the 
                                     // output of the predictor.
reg     [4:0]     output_precision_shift;    // offset of 1st bit to be used for output precision.
reg     [5:0]     output_precision_1st_output_bit_shift;    // Defines the 1st bit which is actually would be output to  
                                     // the DAC = output_precision_shift + output_precision_size
reg    [31:0]     output_precision_and_value;    // Value to and the shifted result to obtain the correct bit
                                     // size as specified in output_precision_size
                                     // (essentially:
                                     // output_precision_and_value = (1 << output_precision_size) - 1
reg     [31:0]    output_precision_current_value;    // The current value of the PWM output shaper.
                                     //
                                     // 
...
reg               second_integrator_output_enable;    // is second integration output enabled?
                                     // (if not, it is just the copy of the
                                     //  1st).
reg               second_integrator_enable;    // If not enabled, it just retains its last value.
                                


reg               continuous;    // Indicates whether continuous operation mode is running
reg               continuous_input_select;    // Indicates which input is selected for the continuous input select.
reg               input_special;    // Indicates whether the input is spatially manipulated before use or not.
reg               pre_dither_manual_enable;    // "0" = normal input to dither output (= output of pid).
                                     // "1" = manual input to dither output (= manual register value, pid result is discarded)
reg     [31:0]    pre_dither_manual_value;
reg     [31:0]    pre_dither_manual_value_sync_in;
    // Value of input to dither's output, in case pre-dither manual is enabled (see line above).


reg               sw_reset_request;    // S/W reset signal                                
...
reg               invert_y_n;
reg               invert_output;
reg               y_input_diff_enable;    // Indicates that y_input is not one of the inputs, but rather the difference between them
...
assign inputSelection = (input_special ? w_y_input_special : y_input);

assign ditherInputSelection = (dither_input_polarity == 0 ? inputSelection : -(inputSelection));
...
always@(posedge reg_clk or negedge rst)
    ...
    begin
        ...
        pre_dither_manual_value_sync_in <= 0;                            
	     ...
	     //
		  ...
		  //
        pre_dither_manual_value_sync_in <= i_pre_dither_manual_value;
        if  (sw_reset_request)
            begin
                sw_reset_request <= 0;
            end
        ...
        begin
            ...
            begin
                ...
                second_integrator_output_enable <= i_config[30];
...
always@(posedge reg_clk or negedge rst)
    if(!rst)begin
            ...
            out_offset_plus_dither_amplitude <= 0;
            output_shift_2nd <= 0;
            i0_shift <= 0;
            i0_2nd <= 0;
...
always@(posedge logic_clk or negedge rst)
    if (!rst) begin
        ...
        pre_dither_manual_value <= pre_dither_manual_value_sync_in;
        ...
        `STATE_1:    begin
            ...
            begin
                ...
                //
                y_average_sum <= (input_special ? w_y_input_special : y_input);
            end
            y_input_for_yn <= (input_special ? w_y_input_special : y_input);
            y_input_for_out <= y_input;
            y_input_square_for_out <= (input_special ? w_y_input_special : y_input);
...
assign    o_q0_q4      = {r4[15:0], r0[15:0]};
assign    o_q1_q5      = {r5[15:0], r1[15:0]};
assign    o_q2_q6      = {r6[15:0], r2[15:0]};
assign    o_q3_q7      = {r7[15:0], r3[15:0]};
...
assign    o_pre_dither_manual_value       = pre_dither_manual_value_sync_in;
...
begin
    ...
    begin
        ...
        begin
            ...
            if (i3_cs[1])
                rec_read_addr <= i3_data_in[9:0];

            if (i3_cs[2])
                rec_interval <= (i3_data_in == 32'b0) ? 32'd1 : i3_data_in;
            end
...
begin
    ...
    begin
        ...
        begin
            ...
            begin
                rec_tick_counter <= rec_tick_counter + 32'b1;
            end
        end
    end
end
